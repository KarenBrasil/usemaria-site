              ...
              <div className="p-5 md:p-6 flex flex-col lg:flex-row gap-8 border-t border-zinc-100 bg-zinc-50/30">
                ...
                <div className="flex-1 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-zinc-50/80 rounded-xl p-5 border border-zinc-200 flex gap-4 items-start">
                       <div className="bg-white p-2.5 rounded-lg text-zinc-600 shrink-0 mt-0.5 border border-zinc-200 shadow-sm">
                         <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                       </div>
                       <div className="w-full">
                          <h4 className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Contato do Cliente</h4>
                          <div className="text-sm font-medium text-zinc-800 leading-relaxed mb-3">
                            <p>{order.customer?.email || 'Sem e-mail'}</p>
                            <p className="font-mono text-zinc-500">{order.customer?.phone || 'Sem telefone'}</p>
                          </div>
                          
                          {order.customer?.phone && (
                            <a 
                              href={`https://wa.me/${order.customer.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Olá ${order.customer.name.split(' ')[0]}! Aqui é da Use Maria, referente ao seu pedido #${order.id.slice(-6).toUpperCase()}.`)}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="w-full bg-[#25D366] text-white font-bold tracking-wide text-xs py-2 px-3 rounded-lg hover:bg-[#1ebd5b] hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2 shadow-sm"
                            >
